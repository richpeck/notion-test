...
class Image
  ...
  def initialize filename, properties, *arguments
    ...
    defaults = {
      Color:         "#3498DB",
      Dark?:         "no", # => old version (need to remove)
      Dark:          "no",
      Name:          nil,
      Initials:      nil,
      Memory:        nil,
      Date:          nil,
      HeartColor:    "#FFBFBF", # => RPECK 26/01/2020
      BirdsColor:    "#ac8cc9", # => RPECK 26/01/2020
      InitialsColor: "#17022F"  # => RPECK 27/01/2020
    }

    # => Merge defaults with properties
    # => Fails if properties is nil, so need to ensure we have logic gate
    @properties = properties.nil? ? defaults : defaults.merge(properties)
